...
describe("Admin/CreateCategory (unit) — tests uncover bugs first", () => {
  test("loads categories on mount and renders table rows (no React key warnings)", async () => {
    ...
    mockedAxios.get.mockResolvedValueOnce({
      data: { success: true, category: [{ _id: "a1", name: "Phones" }, { _id: "a2", name: "Laptops" }] },
    });

    renderPage();

    await waitFor(() => expect(mockedAxios.get).toHaveBeenCalledWith("/api/v1/category/get-category"));
  ...
  test("getAllCategory rejected → shows correctly spelled toast", async () => {
    ...
    await waitFor(() => expect(toastError).toHaveBeenCalledWith("Something went wrong in getting category"));
  });

  test("getAllCategory returns success:false → shows error toast and does not render rows", async () => {
    mockedAxios.get.mockResolvedValueOnce({
      data: { success: false, message: "API down", category: [{ _id: "x", name: "ShouldNotRender" }] },
    });
  ...
  test("create category success → posts, success toast (uses name), refetches list", async () => {
    ...
    await waitFor(() =>
      expect(mockedAxios.post).toHaveBeenCalledWith("/api/v1/category/create-category", { name: "NewCat" })
    );
  ...
  test("create category API returns success:false → shows error toast with API message", async () => {
    mockedAxios.get.mockResolvedValueOnce({ data: { success: true, category: [] } });
    mockedAxios.post.mockResolvedValueOnce({ data: { success: false, message: "Duplicate" } });

    renderPage();

    fireEvent.change(await screen.findByPlaceholderText("Category name"), { target: { value: "Phones" } });
    fireEvent.submit(screen.getByTestId("category-form"));
  ...
  test("create category rejected → shows correctly spelled toast", async () => {
    ...
    fireEvent.change(await screen.findByPlaceholderText("Category name"), { target: { value: "X" } });
    fireEvent.submit(screen.getByTestId("category-form"));

    await waitFor(() => expect(toastError).toHaveBeenCalledWith("Something went wrong in input form"));
  });

  test("click Edit opens modal (AntD v5 `open` prop), update success closes modal, toasts, refetches", async () => {
    mockedAxios.get.mockResolvedValueOnce({ data: { success: true, category: [{ _id: "a1", name: "Phones" }] } });
    ...
    const modalInput = within(modal).getByTestId("category-form-input") as HTMLInputElement;
    ...
    await waitFor(() =>
      expect(mockedAxios.put).toHaveBeenCalledWith("/api/v1/category/update-category/a1", { name: "Gadgets" })
    );

    await waitFor(() => expect(toastSuccess).toHaveBeenCalledWith("Gadgets is updated"));
  ...
  test("update API returns success:false → shows error toast", async () => {
    mockedAxios.get.mockResolvedValueOnce({ data: { success: true, category: [{ _id: "a1", name: "Phones" }] } });
    mockedAxios.put.mockResolvedValueOnce({ data: { success: false, message: "No permission" } });
    ...
    await waitFor(() => expect(toastError).toHaveBeenCalledWith("No permission"));
  });
  ...
  test("delete success → toasts and refetches", async () => {
    mockedAxios.get
      .mockResolvedValueOnce({ data: { success: true, category: [{ _id: "a1", name: "Phones" }] } })
    ...
    await waitFor(() => expect(mockedAxios.delete).toHaveBeenCalledWith("/api/v1/category/delete-category/a1"));
  ...
  test("delete API returns success:false → shows error toast", async () => {
    mockedAxios.get.mockResolvedValueOnce({ data: { success: true, category: [{ _id: "a1", name: "Phones" }] } });
    mockedAxios.delete.mockResolvedValueOnce({ data: { success: false, message: "Protected" } });
