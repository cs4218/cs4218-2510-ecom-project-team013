...
jest.mock("antd", () => {
  ...
  const Option = ({ value, children }: any) =>
    React.createElement("option", { value, "data-testid": `option-${value}` }, children);
...
describe("Admin/CreateProduct (unit)", () => {
  ...
  test("loads categories on mount and renders options", async () => {
    ...
    mockedAxios.get.mockResolvedValueOnce({ data: { success: true, category: cats } });
  ...
  test("shows toast on category fetch error (repo’s typo message)", async () => {
    ...
    await waitFor(() => {
      expect(toastError).toHaveBeenCalledWith("Something went wrong in getting category");
    });
  });

  test("category fetch returns success:false → no options populated and error toast", async () => {
    mockedAxios.get.mockResolvedValueOnce({ data: { success: false, category: [{ _id: "cX", name: "X" }] } });
  ...
  test("fetch categories is called exactly once on mount", async () => {
    mockedAxios.get.mockResolvedValueOnce({ data: { success: true, category: [] } });
  ...
  test("select placeholders are disabled and value changes on selection", async () => {
    ...
    const catPlaceholder = (catSelect as HTMLSelectElement).querySelector("option[value='']");
    ...
    await waitFor(() => expect((catSelect as HTMLSelectElement).value).toBe("c9"));

    // Shipping
    const shipSelect = screen.getByTestId("shipping-select") as HTMLSelectElement;
  ...
  test("select category & shipping, fill fields, upload file → FormData contains expected values", async () => {
    ...
    mockedAxios.post.mockResolvedValueOnce({ data: { success: true, message: "ok" } });
    ...
    fireEvent.change(screen.getByPlaceholderText("write a name"), { target: { value: "Pixel 9" } });
    fireEvent.change(screen.getByPlaceholderText("write a description"), { target: { value: "Nice phone" } });
    fireEvent.change(screen.getByPlaceholderText("write a Price"), { target: { value: "999" } });
    fireEvent.change(screen.getByPlaceholderText("write a quantity"), { target: { value: "3" } });
  ...
  test("when no photo selected, 'photo' is not appended to FormData", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("write a name"), { target: { value: "NoPic" } });
    fireEvent.change(screen.getByPlaceholderText("write a description"), { target: { value: "Desc" } });
    fireEvent.change(screen.getByPlaceholderText("write a Price"), { target: { value: "3" } });
  ...
  test("renders uploaded photo name and preview image", async () => {
    ...
    const file = new File([new Uint8Array([7, 8])], "myimg.jpg", { type: "image/jpeg" });
  ...
  test("on successful create: shows success toast and navigates", async () => {
    mockedAxios.get.mockResolvedValueOnce({ data: { success: true, category: [] } });
    mockedAxios.post.mockResolvedValueOnce({ data: { success: true, message: "Created" } });

    renderPage();

    fireEvent.change(screen.getByPlaceholderText("write a name"), { target: { value: "X" } });
    fireEvent.change(screen.getByPlaceholderText("write a description"), { target: { value: "Y" } });
    fireEvent.change(screen.getByPlaceholderText("write a Price"), { target: { value: "1" } });
    fireEvent.change(screen.getByPlaceholderText("write a quantity"), { target: { value: "2" } });
  ...
  test("on create failure (success:false): shows error toast with API message and does not navigate", async () => {
    mockedAxios.get.mockResolvedValueOnce({ data: { success: true, category: [] } });
    mockedAxios.post.mockResolvedValueOnce({ data: { success: false, message: "Invalid input" } });
  ...
  test("submits even if category not chosen (category empty string submitted)", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText("write a name"), { target: { value: "NoCat" } });
    fireEvent.change(screen.getByPlaceholderText("write a description"), { target: { value: "Desc" } });
