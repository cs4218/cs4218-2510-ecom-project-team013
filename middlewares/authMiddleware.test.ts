...
describe("authMiddleware", () => {
  ...
  describe("Integration Scenarios", () => {
    it("should handle requireSignIn followed by isAdmin for admin user", async () => {
      const token = "valid-token";
      const decoded = { _id: "admin123" };
      mockRequest.headers = { authorization: `Bearer ${token}` };
      (JWT.verify as jest.Mock).mockReturnValue(decoded);
      (userModel.findById as jest.Mock).mockResolvedValue({ role: 1 });
    ...
    it("should reject non-admin user in isAdmin even if authenticated", async () => {
      const token = "valid-token";
      const decoded = { _id: "user123" };
      mockRequest.headers = { authorization: token };
      (JWT.verify as jest.Mock).mockReturnValue(decoded);
